    ...
    const animate = () => {
      ...
      for (let i = 0; i < dots.length; i++) {
        ...
        if (mouse.active) {
          ...
          if (dist < INFLUENCE_RADIUS) {
            ...
            if (isDark) {
              ...
              const targetX = dot.originX + (dx / dist) * MAX_DISPLACEMENT * force * force;
              const targetY = dot.originY + (dy / dist) * MAX_DISPLACEMENT * force * force;
              ...
              const targetX = dot.originX - (dx / dist) * MAX_DISPLACEMENT * 1.5 * repelForce;
              const targetY = dot.originY - (dy / dist) * MAX_DISPLACEMENT * 1.5 * repelForce;
              dot.x += (targetX - dot.x) * EASE_SPEED;
              dot.y += (targetY - dot.y) * EASE_SPEED;
              targetScale = 1 + repelForce * 2.5; // zoom up to 3.5x at center
            }
          } else {
        ...
        const fillColor = isDark
          ...
          : `rgba(0, 0, 0, ${Math.min(alpha + (dot.scale - 1) * 0.06, 0.3)})`;
